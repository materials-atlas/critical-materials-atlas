...
<article style="max-width:1040px">
  ...
  <p class="muted" style="margin-top:0">A fair worry: 24-year volatility might hide episodic spikes. With <b>non-overlapping</b> windows the direction holds at every length &mdash; bigger markets are calmer, and by-product status never comes close to mattering &mdash; <span id="horizverdict"></span></p>
...
Promise.all([fetch('out/price_volatility.json').then(r=>r.json()),
  ...
  { const hs=S.horizons||[]; const sig=hs.filter(h=>h.size_p<0.05).length; const lo=Math.min(...hs.map(h=>h.size_p)), hi=Math.max(...hs.map(h=>h.size_p)); document.getElementById('horizverdict').innerHTML = (sig===hs.length) ? 'and the size effect is significant at all '+hs.length+' (p '+lo.toFixed(3)+'&ndash;'+hi.toFixed(3)+').' : 'but the size effect clears the 5% line at only '+sig+' of '+hs.length+' (p '+lo.toFixed(3)+'&ndash;'+hi.toFixed(3)+'), so read it as consistent in sign, not significant everywhere.'; }
